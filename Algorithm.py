...
def find_optimal(det_range, limits, grid, vor4, max_len, precision, threshold):
    """
    ...
    for det in det_range:
        ...
        for diag_el in list_diag_el:
            ...
            for indices in product(range(max_num_col3), range(max_num_col3), range(max_num_col2),
                ...
                sub_grid_int = (scale * sub_grid).astype(int)
               
                basis = LLL.reduction(IntegerMatrix.from_matrix(sub_grid_int.tolist()))
                rows, cols = basis.nrows, basis.ncols
                sub_grid_LLL = 1/scale * np.array([[basis[i, j] for j in range(cols)] for i in range(rows)])
                #sub_grid_LLL = LLL_2(sub_grid, 0.75)
 
                centers = lattice_points_no_central_symmetry(sub_grid_LLL, limits, 5, vor4.max_len)
                min_dist_mat = float('inf')
   
                #min_dist_mat = 5 * vor4.max_len
                ...
                for center in centers:
                    ...
                    #if dist > min_dist_mat + 2 * vor4.max_len: continue

                    if min_dist_mat < threshold: break

                if min_dist_mat < threshold: continue
...
def lattice_points_no_central_symmetry_3(basis, limits, coeff_dist, max_len):
    """
    Генерирует точки решетки, заданной базисом, без центральносимметричных точек,
    сразу исключая симметричные коэффициенты.
    ...
    if len(points) >= 2:
        for i in points:
            dist = distance.euclidean(i, [0.0, 0.0, 0.0, 0.0])
            
            if dist< min_dist:
                min_dist = dist
                index = i
        #dist_matrix = distance.pdist(points)
        #min_dist_2 = np.min(dist_matrix)
        #print(dist_matrix)
...
def find_optimal_3(det_range, limits, grid, vor4, max_len):
    """
    Основная функция для выполнения алгоритма поиска матриц и расстояний.
    ...
    for det in det_range:
        ...
        for diag_el in list_diag_el:
            ...
            for indices in product(range(max_num_col3), range(max_num_col3), range(max_num_col2),
               ...
               # print(sub_grid_LLL)
                d, center = lattice_points_no_central_symmetry_2(sub_grid_LLL, limits, 3, vor4.max_len)
                
                
                min_dist_mat = 4 * vor4.max_len
